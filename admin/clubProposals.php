...
<html lang="en">
    <head>
        ...
        <?php
            ...
            $sql = "SELECT proposalId, title, p.description, reasoning, startDate, personInCharge,
            p.studentId, s.name, status, comment, submitDate, proposalFile, p.clubId, activityId
            FROM proposal AS p
    ...
    <body>
        ...
        <main id="mainMargin">
            <div id="mySidenav" class="sidenav" style="width :50px;">
               ...
               <div class="container">
                <h1 class="text-center p-3">Club Proposals</h1>
                        ...
                        <table class="table table-striped table-dark" id="thetable">
                            <thead>
                                <tr>
                                    ...
                                    <th onclick="sortTable(1)" scope="col"> Club Name</th>
                                    <th onclick="sortTable(2)" scope="col"> Description </th>
                                    <th onclick="sortTable(3)" scope="col"> Reasoning </th>
                                    <th onclick="sortTable(4)" scope="col"> Submit Date </th>
                                    <th onclick="sortTable(5)" scope="col"> President ID </th>
                                    <th onclick="sortTable(6)" scope="col"> Submitted By </th>
                                    <th onclick="sortTable(7)" scope="col"> Status </th>
                                    <th onclick="sortTable(8)" scope="col"> Comment</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                    foreach ($_SESSION['proposals'] as $index => $arrayRow) {
                                        echo '<tr>';
                                        echo '<td>'. $arrayRow['proposalId'] .'</td>';
                                        echo '<td>'. $arrayRow['title'] .'</td>';
                                        echo '<td>'. $arrayRow['description'] .'</td>';
                                        echo '<td>'. $arrayRow['reasoning'] .'</td>';
                                        ...
                                        echo '<td>'. $arrayRow['comment'] .'</td>';
                                        echo '<td>';
                                        switch($arrayRow['status']){
                                            case "Pending":
                                                echo '<button class="btn btn-outline-warning" 
                                                onclick="review2(\'' .$arrayRow['proposalId'].'\',\'../Proposals/'.$arrayRow['proposalFile'].'\')"> 
                                                Review </button>';
                                                break;
                                            case "Published":
                                                echo '<a href="../club/edit.php?id='.$arrayRow['clubId'].'"><button 
                                                class="btn btn-outline-success"
                                                type="button"> Edit 
                                                </button></a>';
                                                echo '<button class="btn btn-outline-success"
                                                onclick="view2(\'' . htmlspecialchars($arrayRow['comment'], ENT_QUOTES).'\')"> 
                                                View </button>';
                                                break;
                                            case "Approved":
                                            case "Rejected":
            ...
            <div id="popup" class="align-items-center justify-content-center" onclick="closePopUp()" style="display: none; flex-direction: column;">
                <form class="contact-form d-flex flex-column p-3 col-10 col-lg-8 bg-customGrey" action="proposalReview.php" method="post" onclick="preventClose(event)">
                    ...
                    <input type="hidden" value="club" id="proposal" name="proposal">
